...
static int
send_response_200_dynamic(int fd, char *basename, DIR* pDir, int id)
{
  //used for reading the directory
  struct dirent *pDirent;  

  //in order to dynamically generate the response, and be able to easily get the content-length
  //i divide the static part of the HTML into vars which I then append to my dynamic string
  char start[] = "<html>\r\n<head>\r\n<title> Subdirectory </title>\r\n</head>\r\n<body>\r\n<ul>\r\n";
  char end[] = "</ul>\r\n</body>\r\n</html>\r\n";            
  char anchor_tag[] = "<li><a href='";
  char anchor_close[] = "'>";
  char anchor_tag_close[] = "</a></li>\r\n"; 
  char go_back[] = "<li><a href='/index.html'>Go back</a></li>";

  //here I have my dynamic string where I will store the HTML and also add links to each file in the dir
  char* html_dynamic = malloc(sizeof(start)*sizeof(char));
  strcpy(html_dynamic,start);  

  while ((pDirent = readdir(pDir)) != NULL) { //read file by file and ignore "." and ".."
    if(strcmp(pDirent->d_name,"..")!=0 && strcmp(pDirent->d_name,".") != 0){
      //reallocate memory
      html_dynamic = realloc(html_dynamic, strlen(html_dynamic)+1+strlen(anchor_tag)+(strlen(pDirent->d_name)*2)+strlen(anchor_close)+strlen(basename)+strlen(anchor_tag_close)); 
      
      // if terminating symbol is not applied then valgrind gives lots of errors
      // valgrind shows no errors (introduced by my code) and everything
      // works as intended with no memory leaks and no read/write errors 
      html_dynamic[strlen(html_dynamic)] = '\0';
      //append the needed HTML
      strcat(html_dynamic, anchor_tag);
      strcat(html_dynamic, basename);
      strcat(html_dynamic, pDirent->d_name);
      strcat(html_dynamic, anchor_close);
      strcat(html_dynamic, pDirent->d_name);
      strcat(html_dynamic, anchor_tag_close);
    }
  }
  //append the final html
  html_dynamic = realloc(html_dynamic, strlen(html_dynamic)+sizeof(end)+sizeof(go_back));
  strcat(html_dynamic, go_back);
  strcat(html_dynamic, end);
  
  //now deal with the headers
  char headers[BUFLEN+strlen(html_dynamic)];
  sprintf(headers, "HTTP/1.1 200 OK\r\n"
                   "Content-Type: text/html\r\n"
                   "Content-Length: %lu\r\n\r\n", strlen(html_dynamic));

  strcat(headers,html_dynamic); 
  
  if(send_response(fd, headers, strlen(headers)) == -1){
    free(html_dynamic); // free up the memory
    return -1;
  }
  
  printf("responder %d: 200 %s (%lu bytes)\n", id, basename, strlen(html_dynamic));
...
static int
send_response_307(int fd, char *redirect, int id)
{
  char headers[65535];
  char address_redirect[65535];
  
  // check for the ending
  if(redirect[strlen(redirect)] == '/'){
    strcpy(address_redirect,"index.html");
  } else{
    strcpy(address_redirect,"/index.html");
  }
  

  sprintf(headers, "HTTP/1.1 307 Temporary Redirect\r\n"
                  "Location: %s\r\n"
                  "Content-Type: text/html\r\n"
                  "Content-Length: 99\r\n"
                  "\r\n"
                  ...
                  "</body>\r\n"                             //  9
                  "</html>\r\n"                             //  9
         , address_redirect);                                                 // 99 total


  
  if(send_response(fd, headers, strlen(headers)) == -1) return -1;

  printf("responder %d: 307 %s (%d bytes)\n", id, address_redirect, (int) strlen(headers));
  return 0;
}
...
response_thread(void *arg)
{
  ...
  while ((fd = wq_get(wq)) != -1) {
    ...
    while (1) {
      ...
      //check if index.html is in the current dir and redirect 
      char new_name[1024];
      sprintf(filename, "website%s", basename);
      strcpy(new_name, filename);
      strcat(new_name, "index.html");
      if((access(new_name, F_OK)) != -1 ) {
          if(send_response_307(fd, basename, id) == -1){
            free(headers);
            break;
          }
      } else { // if not then generate a dynamic HTML with all the files in the dir
          DIR *pDir;
          if((pDir = opendir(filename)) != NULL){
            if(send_response_200_dynamic(fd, basename, pDir, id) == -1){
              free(headers);
              closedir(pDir);
              break;
            }
